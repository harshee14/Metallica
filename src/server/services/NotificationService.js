...
module.exports.process = (action, registry, cb) => {
        const logger = log4js.getLogger('NotificationService');
        if(!action || !action.service) {
            logger.error("Action not specified for NotificationService");
        ...
        return request
            .get(`http://${service.ip}:${service.port}/notification`)
